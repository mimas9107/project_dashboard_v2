...
class ProjectManager:
    ...
    def _build_tree(self, path: Path, depth: int) -> Optional[Dict]:
        ...
        try:
            ...
            tree["children"] = sorted(
                entries, key=lambda x: (x["type"] != "folder", x["name"].lower())
            )
    ...
    def get_readme_content(self, project_name: str) -> str:
        """
        獲取專案的完整 README.md 內容

        Args:
            project_name: 專案名稱

        Returns:
            README.md 的完整內容，若不存在則回傳提示訊息
        """
        project_path = self.validate_project_path(project_name)
        readme_path = project_path / "README.md"

        if not readme_path.exists():
            return "此專案尚無 README.md 檔案。"

        try:
            with open(readme_path, "r", encoding="utf-8") as f:
                return f.read()
        except Exception as e:
            return f"讀取 README.md 時發生錯誤：{e}"
        ...
        for project in self.list_all_projects():
            ...
            if language in languages:
                matching_projects.append(
                    {**project, "language_percentage": languages[language]}
                )

        # 按語言佔比排序
        return sorted(
            matching_projects, key=lambda x: x["language_percentage"], reverse=True
        )
    ...
    def get_modified_projects(self) -> List[Dict]:
        ...
        for project in self.list_all_projects():
            ...
            if status == "Modified":
                modified.append({**project, "git_detail": detail})
    ...
    def batch_git_status(self) -> Dict[str, List[str]]:
        ...
        Returns:
            按狀態分組的專案字典
        """
        status_groups = {"Clean": [], "Modified": [], "Not a Git repo": [], "Error": []}
    ...
    def open_in_editor(
        self, project_name: str, editor: str = "code"
    ) -> Tuple[bool, str]:
        """
        在編輯器中開啟專案
